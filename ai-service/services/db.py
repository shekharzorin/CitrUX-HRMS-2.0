...
def _serialize_row(row):
    """Convert a RealDictRow (or plain dict) to a JSON-safe dict."""
    if row is None:
        return None
    result = {}
...
def _serialize_rows(rows):
    """Convert a list of RealDictRow objects to JSON-safe dicts."""
    return [_serialize_row(row) for row in rows] if rows else []
...
class DBService:
    def __init__(self):
        ...
        elif "&pgbouncer=true" in self.conn_str:
            self.conn_str = self.conn_str.replace("&pgbouncer=true", "")
    ...
    def get_leave_balance(self, user_id: str, company_id: str = None):
        ...
        try:
            ...
            cur.execute("""
                ...
                WHERE lb."userId" = %s AND (%s IS NULL OR u."companyId" = %s)
    ...
    def get_pending_approvals(self, company_id: str = None):
        ...
        try:
            ...
            cur.execute("""
                ...
                FROM "LeaveRequest" lr
                JOIN "User" u ON lr."userId" = u.id
                WHERE (%s IS NULL OR u."companyId" = %s) AND lr.status = 'PENDING'
            """, (company_id, company_id))
            ...
            cur.execute("""
                ...
                WHERE (%s IS NULL OR u."companyId" = %s) AND ec.status = 'PENDING'
            """, (company_id, company_id))
